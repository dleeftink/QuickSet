...
- [`.add()`](#add-uint-value) , [`.put()`](#put-uint-value) and [`.unique()`](#unique-uints) overwrite previous values and **do not** update the top-k window when called.
- [`.sum()`](#sum-uint-value) and [`.batch()`](#batch-uints-values) maintain previous values and **do** update the top-k window when called.
