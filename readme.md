...
Methods can be mixed and matched to your liking, but may yield unwanted results if used without care: `.add()`, `.put()` & `.unique()` overwrite previous values and **do not** update the top-k window; use `.sum()` & `.batch()` if you want updates to be reflected in the top-k window when inserting new integers (see the [`put`](#put) example for why this is useful).
