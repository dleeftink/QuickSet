...
> **IMPORTANT: Setting `fifo` to `true` overwrites the default [`ranking`](#rankers) method. To reset the ranking algorithm `.minsum()` and `.winsum()`, initiate a [`QuickSet`](#new-quickset-config).**
