export default class QuickSet {
  ...
  constructor({
    ...
    if (span > 2 ** 28) throw Error("Expected integer beyond memory range");

    if (high > 2 ** 32 - 1)
      throw Error("Expected count beyond frequency range");

    if (slot > 16) throw Error("Rank slots performance degradation > 16");
  ...
  #minsum(uint, val = 1) {
    ...
    if (uint < this.clip || uint > this.span || invalid(uint) || invalid(val))
      return;
